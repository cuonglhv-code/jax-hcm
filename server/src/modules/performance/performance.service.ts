...
export const performanceService = {
  ...
  async activateCycle(id: string) {
    return await db.transaction(async trx => {
      await trx('appraisal_cycles').update({ status: 'completed', updated_at: db.fn.now() })
  ...
  async listAppraisals(
    filters: { cycleId?: string; employeeId?: string; managerId?: string; status?: string },
    page = 1,
    limit = 20
  ) {
  ...
  async getAppraisal(id: string) {
    ...
    const responses = await db('appraisal_responses')
      ...
      .select(
        'appraisal_responses.*',
        'appraisal_questions.text as question_text',
        'appraisal_questions.rating_type'
      )
  ...
  async advanceAppraisal(id: string, action: string, responderUserId: string) {
    ...
    if (action === 'submit_self') {
      if (appraisal.status !== 'draft')
        throw new AppError(400, 'Appraisal must be in draft to self-submit')
      const emp = await db('employees').where({ id: appraisal.employee_id }).first()
      if (emp?.user_id !== responderUserId)
        throw new AppError(403, 'Not authorized to submit self appraisal')
      updates.status = 'submitted'
      updates.self_submitted_at = db.fn.now()
    } else if (action === 'submit_manager') {
      if (appraisal.status !== 'submitted')
        throw new AppError(400, 'Appraisal must be submitted to review')
      const mgr = await db('employees').where({ id: appraisal.manager_id }).first()
      if (mgr?.user_id !== responderUserId)
        throw new AppError(403, 'Not authorized to submit manager review')
      updates.status = 'reviewed'
      updates.manager_submitted_at = db.fn.now()
    } else if (action === 'acknowledge') {
      if (appraisal.status !== 'reviewed')
        throw new AppError(400, 'Appraisal must be reviewed to acknowledge')
      const emp = await db('employees').where({ id: appraisal.employee_id }).first()
      if (emp?.user_id !== responderUserId)
        throw new AppError(403, 'Not authorized to acknowledge appraisal')
  ...
  async listGoals(employeeId: string, cycleId?: string) {
    const query = db('goals').where({ employee_id: employeeId }).orderBy('created_at', 'desc')

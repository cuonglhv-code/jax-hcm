...
export const learningService = {
  // ─── Courses ─────────────────────────────────────────────────────────
  async listCourses(filters: { type?: string, isMandatory?: string, search?: string }, page = 1, limit = 20) {
    const query = db('courses').whereNull('deleted_at').orderBy('title', 'asc');

    if (filters.type) query.where('type', filters.type);
    if (filters.isMandatory !== undefined) query.where('is_mandatory', filters.isMandatory === 'true');
  ...
  async updateEnrolmentStatus(id: string, status: string) {
    return await db.transaction(async trx => {
      ...
      const [updated] = await trx('course_enrolments')
        .where({ id })
        .update(updates)
        .returning('*');
  ...
  async listEnrolmentsByCourse(courseId: string, page = 1, limit = 20) {
    const query = db('course_enrolments')
      ...
      .select('course_enrolments.*', 'employees.first_name', 'employees.last_name', 'employees.email')
  ...
  async getMandatoryTrainingStatus(employeeId: string) {
    ...
    const enrolments = await db('course_enrolments')
      .where({ employee_id: employeeId });

    const statusList = rules.map(rule => {
      // Find latest completed enrolment for this course
      const courseEnrolments = enrolments.filter(e => e.course_id === rule.course_id && e.status === 'completed');
      courseEnrolments.sort((a, b) => new Date(b.completed_at).getTime() - new Date(a.completed_at).getTime());
      ...
      if (latest) {
        if (rule.renewal_period_days) {
          ...
          const expiryDate = new Date(completedAt.getTime() + rule.renewal_period_days * 24 * 60 * 60 * 1000);
         ...
         const active = enrolments.find(e => e.course_id === rule.course_id && ['enrolled', 'in_progress'].includes(e.status));

...
export const leaveService = {
  ...
  async getLeaveBalance(employeeId: string, year: number) {
    ...
    const requests = await db('leave_requests')
      ...
      .andWhere(db.raw(`EXTRACT(YEAR FROM start_date) = ?`, [year]));

    return entitlements.map(ent => {
      // Sum requested days for this leave type in this year
      const used = requests
        .filter(r => r.leave_type_id === ent.leave_type_id)
        .reduce((sum, r) => sum + parseFloat(r.days as string), 0);
      
      const total = parseFloat(ent.total_days as string) + parseFloat(ent.carry_over_days as string);
      return {
  ...
  // ─── Leave Requests ──────────────────────────────────────────────────
  async listLeaveRequests(filters: { employeeId?: string, status?: string, year?: string }, page = 1, limit = 20) {
    ...
    if (filters.year) query.where(db.raw(`EXTRACT(YEAR FROM leave_requests.start_date) = ?`, [filters.year]));
        ...
        const overlaps = await trx('leave_requests')
          ...
          .andWhere(function() {
            this.where('start_date', '<=', request.end_date)
                .andWhere('end_date', '>=', request.start_date);
          });
  ...
  async cancelLeaveRequest(id: string, requesterEmployeeId: string) {
    return await db.transaction(async (trx) => {
      const request = await trx('leave_requests').where({ id }).first();
      if (!request) throw new AppError(404, 'Leave request not found');
      if (request.employee_id !== requesterEmployeeId) throw new AppError(403, 'Not authorized to cancel this request');

...
export const recruitmentService = {
  ...
  async listCandidates(page = 1, limit = 20, search?: string) {
    const query = db('candidates').select('*').orderBy('created_at', 'desc')
  ...
  async advanceStage(applicationId: string, stage: string, notes?: string) {
    ...
    return await db.transaction(async trx => {
      ...
      if (stage === 'hired') {
        ...
        const req = await trx('job_requisitions').where({ id: updatedApp.requisition_id }).first()
  ...
  async getOnboardingChecklist(employeeId: string) {
    const checklist = await db('onboarding_checklists').where({ employee_id: employeeId }).first()
